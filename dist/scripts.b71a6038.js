...
function chooseChallenger() {
  var playersLeft = players.filter(function (player) {
    return player !== king;
  });
...
function playersLeft() {
  var noKing = players.filter(function (player) {
    return player !== king;
  });
  var noChallengers = noKing.filter(function (player) {
...
var currentQueue = playersLeft();
updateScores();
var kingName = document.querySelector('.king .name');
...
var kingScore = document.querySelector('.king .score');
var challengerScore = document.querySelector('.challenger .score');
...
document.querySelector('.king').addEventListener('click', function () {
  ...
  if (currentScore[0].points === pointLimit) {
    ...
    if (king.score === pointLimit) {
      alert("winner winner, ".concat(king.name, " wins!!"));
      resetGame();
    }
...
document.querySelector('.challenger').addEventListener('click', function () {
  ...
  if (currentScore[1].points === pointLimit) {
    ...
    if (challenger.score === pointLimit) {
      alert("winner winner, ".concat(challenger.name, " wins!!"));
      resetGame();
    }
...
function updateScores() {
  ...
  serveCounter = 0;
  var queueHTML = '';
  queueHTML += "<li>".concat(king.name, " - score: ").concat(king.score, "</li>");
  queueHTML += "<li>".concat(challenger.name, " - score: ").concat(challenger.score, "</li>");
  queueHTML += "<li class=\"divider\">========</li>";

  for (var i = 0; i < currentQueue.length; i++) {
    queueHTML += "<li>".concat(currentQueue[i].name, " - score: ").concat(currentQueue[i].score, "</li>");
  }
